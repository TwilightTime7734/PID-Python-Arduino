...
class ModbusApp(HardwareStateMixin):
    ...
    def run(self) -> None:
        ...
        def record_auto_session_sample(sample) -> None:
            ...
            signed_delta = axis_value - self.auto_event_baseline
            abs_delta = abs(signed_delta)
            if abs_delta > self.auto_event_abs_peak_delta:
                self.auto_event_abs_peak_delta = float(abs_delta)
                self.auto_event_signed_peak_delta = float(signed_delta)
            ...
            if self.auto_controller is not None:
                cfg = self.auto_controller.config
                ...
                if emergency_peak_deg > 0 and abs_delta >= emergency_peak_deg:
                    ...
                    return
            if self.auto_controller is not None and self.auto_pulse_inflight and within_hold_window:
        ...
        def lower_throttle_for_abort() -> None:
            ...
            if len(channels) <= THROTTLE_CHANNEL_INDEX:
                return
            target = max(1000, min(2000, int(self.auto_config.abort_throttle_us)))
            channels[THROTTLE_CHANNEL_INDEX] = min(channels[THROTTLE_CHANNEL_INDEX], target)
            apply_auto_base_outputs(channels, send_update=False)
            self.auto_original_base_outputs = None
            self.beeper_marker_active = False
            if arduino_output_connected():
                queue_live_channel_update(channels.copy(), parse_offset_values_with_defaults())
        ...
        def run_pid_plan_final_write() -> None:
            ...
            yaw = self.pid_plan.yaw_final_pid_ff
            yaw_recommendation = (
            ...
            messagebox.showinfo("PID Plan Complete", completion_text, parent=self.root)
        ...
        def continue_pid_tuning_plan() -> None:
            ...
            if self.pid_plan_phase == "safe_start":
                ...
                if not ensure_disarmed_before_pid_write():
                    self.status.set("Safe-start canceled; disarm before staging PID/FF values.")
                    update_pid_progress_window()
                    return
        ...
        def complete_auto_session(
            ...
            stop_auto_session_runtime(restore_outputs=not lower_throttle)
            if arduino_output_connected():
                try:
        ...
        def auto_abort(reason: str, warning: str = "", continue_pipeline: bool = False) -> None:
            complete_auto_session(AdaptiveSessionState.aborted, reason, warning, lower_throttle=True)
        ...
        def finalize_auto_event() -> None:
            ...
            if self.auto_active_command.calibration:
                axis = self.auto_active_command.axis
                signed_peak = self.auto_event_signed_peak_delta
                abs_peak = self.auto_event_abs_peak_delta
                if abs_peak < self.auto_config.probe_min_response_deg:
                    auto_abort(
                    ...
                    return
                self.auto_axis_output_sign[axis] = 1 if signed_peak >= 0 else -1
                self.auto_controller.record_axis_response(
                    axis,
                    self.auto_active_command.force_us,
                    self.auto_active_command.hold_s,
                    abs_peak,
                )
                self.status.set(
                ...
                if self.pid_plan_fly_log_active and not self.auto_probe_axes_pending:
                    begin_fly_log_marker_off_and_complete()
                return
        ...
        def issue_auto_command(command: AdaptiveCommand) -> None:
            ...
            channel_index = axis_channel_index(command.axis)
            output_sign = 1 if command.calibration else self.auto_axis_output_sign.get(command.axis, 1)
            pwm_direction = command.direction * output_sign
            target = self.base_channel_outputs[channel_index] + (pwm_direction * command.force_us)
        ...
        def issue_next_auto_probe() -> bool:
            if self.auto_controller is None or not self.auto_probe_axes_pending:
                return False
            axis = self.auto_probe_axes_pending.pop(0)
            command = AdaptiveCommand(
                axis=axis,
                direction=1,
                force_us=max(1, int(self.auto_config.probe_force_us)),
                hold_s=max(0.01, float(self.auto_config.probe_hold_s)),
                settle_s=max(0.0, float(self.auto_config.probe_settle_s)),
                recovery=False,
                reason="output sign/gain probe",
                target_peak_deg=max(0.0, float(self.auto_config.probe_target_peak_deg)),
                calibration=True,
            )
            self.status.set(f"Calibrating {axis} output with a small Fly/Log probe.")
        ...
        def run_auto_tick() -> None:
            ...
            if abort:
                auto_abort(abort_reason, continue_pipeline=False)
                return

            calibration_in_progress = bool(self.auto_probe_axes_pending) or (
                self.auto_active_command is not None and self.auto_active_command.calibration
            )
            if calibration_in_progress and self.auto_controller.should_recover(sample.roll_deg, sample.pitch_deg):
                auto_abort(
            ...
            if not self.auto_recovery_mode and self.auto_probe_axes_pending:
                try:
                    if issue_next_auto_probe():
                        schedule_auto_tick()
                        return
                except Exception as exc:
                    auto_abort("Unable to issue Fly/Log calibration probe.", warning=str(exc))
                    return

            if self.pid_plan_fly_log_active:
                ready, stop_reason, warning = self.auto_controller.stop_ready(auto_elapsed_s(now))
                if ready:
                    if self.beeper_marker_active:
                        auto_abort("Fly/Log safety timeout with beeper marker still on.", warning=stop_reason)
                    else:
                        finish_pid_plan_fly_log(stop_reason)
                    return
                schedule_auto_tick()
                return
        ...
        def do_cancel_auto_session() -> None:
            try:
                ...
                if auto_is_running() and self.auto_state != AdaptiveSessionState.import_analyze:
                    complete_auto_session(AdaptiveSessionState.aborted, "Canceled by user.", lower_throttle=True)
                    self.pid_plan_fly_log_active = False
                    refresh_fly_log_button_state()
                    set_auto_button_idle()
